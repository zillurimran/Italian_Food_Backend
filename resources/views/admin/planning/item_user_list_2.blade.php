@foreach ($users  as $user)
    ...
    <li class="select-members-list__item">
        <label class="select-member-block position-relative">
            <input data-id="{{ $item->id }}" data-trello-id="{{ $item->trello_id }}" type="checkbox" name="member_item" class="select-member-block__input select-member-block__input_id2" {{ $checked ? 'checked':'' }} value="{{ $user->id }}" >
